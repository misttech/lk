...
struct virtio_config_ops {
  ...
  void (*kick)(struct virtio_device *dev, uint ring_index);
  void (*set_ring)(struct virtio_device *dev, uint16_t index, uint16_t count, paddr_t pa_desc,
                   paddr_t pa_avail, paddr_t pa_used);

  void (*set_guest_features)(struct virtio_device *dev, uint32_t features);
};
...
struct virtio_device {
  ...
  volatile const struct virtio_config_ops *config;
  void *config_ptr;
...
inline void virtio_reset_device(struct virtio_device *dev) { dev->config->reset(dev); }
...
status_t virtio_alloc_ring(struct virtio_device *dev, uint16_t index, uint16_t count) __NONNULL();
