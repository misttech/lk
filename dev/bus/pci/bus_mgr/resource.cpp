...
status_t resource_allocator::set_range(const resource_range &range, bool prefetchable) {
  LTRACEF("range base %#lx size %#lx type %d prefetchable %d\n", range.base, range.size, range.type,
          prefetchable);
...
status_t resource_allocator::allocate_mmio(bool can_be_64bit, bool prefetchable, uint64_t size,
  ...
  for (;;) {
    ...
    LTRACEF("range base %#lx size %#lx. request size %#lx align %u prefetchable %d can_be_64 %d\n",
...
status_t resource_allocator::allocate_io(uint32_t size, uint8_t align, uint32_t *out) {
  ...
  LTRACEF("range base %#lx size %#lx. request size %#x align %u\n", range.base, range.size, size,
          align);
