...
class device {
 public:
  ...
  template <typename F>
  status_t for_every_capability(F func);
...
// call the provided functor on every capability in this device
template <typename F>
inline status_t device::for_every_capability(F func) {
  status_t err = NO_ERROR;

  capability *c;
  list_for_every_entry (&capability_list_, c, capability, node) {
    err = func(c);
    if (err != NO_ERROR) {
      return err;
    }
