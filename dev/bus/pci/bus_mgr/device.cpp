...
status_t device::probe(pci_location_t loc, bus *parent_bus, device **out_device) {
  ...
  *out_device = nullptr;
  ...
  fbl::AllocChecker ac;
  device *d = new (&ac) device(loc, parent_bus);
  if (!ac.check()) {
...
status_t device::probe_capabilities() {
  ...
  for (;;) {
    ...
    fbl::AllocChecker ac;
    capability *cap = new (&ac) capability;
    if (!ac.check()) {
      return ERR_NO_MEMORY;
    }
    cap->id = cap_id;
...
status_t device::get_bar_alloc_requests(list_node *bar_alloc_requests) {
  ...
  for (auto i = 0; i < 6; i++) {
    ...
    fbl::AllocChecker ac;
    auto request = new (&ac) bar_alloc_request;
    if (!ac.check()) {
      return ERR_NO_MEMORY;
    }
    *request = {};
...
status_t device::assign_resource(bar_alloc_request *request, uint64_t address) {
  char str[14];
  LTRACEF("device at %s resource addr %#lx request:\n", pci_loc_string(loc(), str), address);
  if (LOCAL_TRACE) {
    request->dump();
  }
...
void device::bar_alloc_request::dump() {
  char str[14];
  if (bridge) {
    printf("BAR alloc request %p: bridge %s type %u (%s) pref %d size %#lx align %u\n", this,
           pci_loc_string(dev->loc(), str), type, pci_resource_type_to_str(type), prefetchable,
           size, align);
  } else {
    printf("BAR alloc request %p: device %s type %u (%s) pref %d size %#lx align %u bar %u\n", this,
           pci_loc_string(dev->loc(), str), type, pci_resource_type_to_str(type), prefetchable,
           size, align, bar_num);
  }
}
