...
static int pci_config(int argc, const cmd_args* argv, uint32_t flags) {
  ...
  if (!strcmp(argv[2].str, "dump")) {
    loc.segment = 0;
    loc.bus = static_cast<uint>(argv[3].i);
    loc.dev = static_cast<uint>(argv[4].i);
    loc.fn = static_cast<uint>(argv[5].i);
    ...
    if (header_type == PCI_HEADER_TYPE_STANDARD) {  // type 0
      ...
      switch ((config.type1.prefetchable_memory_base & 0xf)) {
        ...
        case 1:  // 64 bit prefetchable addressing
          printf("prefetchable base=%lx prefetchable limit=%lx\n",
    ...
    loc.dev = static_cast<uint>(argv[4].i);
    loc.fn = static_cast<uint>(argv[5].i);
    offset = static_cast<uint>(argv[6].i);
    ...
    switch (argv[2].str[1]) {
      case 'b': {
        uint8_t value = static_cast<uint>(argv[7].i);
      ...
      case 'h': {
        uint16_t value = static_cast<uint>(argv[7].i);
      ...
      case 'w': {
        uint32_t value = static_cast<uint>(argv[7].i);
...
STATIC_COMMAND("lk_pci", "LK PCI Toolbox", &pci_cmd)
