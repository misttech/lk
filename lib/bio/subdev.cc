...
status_t bio_publish_subdevice(const char *parent_dev, const char *subdev, bnum_t startblock,
  ...
  bdev_t *parent = nullptr;
  subdev_t *sub = nullptr;
  ...
  if (parent->geometry_count && parent->geometry) {
    ...
    const bio_erase_geometry_info_t *geo = nullptr;

    LTRACEF("Searching geometry for region which contains @[0x%lx, 0x%lx)\n", byte_start,
