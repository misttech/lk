...
static int cmd_bio(int argc, const cmd_args *argv, uint32_t flags) {
  ...
  if (!strcmp(argv[1].str, "list")) {
    ...
    zx_instant_mono_t t = current_mono_time();
    ssize_t err = bio_read(dev, (void *)address, offset, len);
    t = current_mono_time() - t;
    ...
    uint8_t *buf = static_cast<uint8_t *>(memalign(MAX_CACHE_LINE, 256));
    ssize_t err = 0;
    while (len > 0) {
      size_t amt = ktl::min<size_t>(256, len);
    ...
    uint32_t request = static_cast<uint32_t>(argv[3].u);
...
static bool is_valid_block(bdev_t *device, bnum_t block_num, uint8_t *pattern,
  ...
  uint8_t *block_contents = static_cast<uint8_t *>(memalign(DMA_ALIGNMENT, device->block_size));
...
static ssize_t write_test(bdev_t *device) {
  uint8_t *test_buffer = static_cast<uint8_t *>(memalign(DMA_ALIGNMENT, device->block_size));
...
static status_t memory_mapped_test(bdev_t *device) {
  ...
  uint8_t *reference_buffer = static_cast<uint8_t *>(memalign(DMA_ALIGNMENT, device->block_size));
  if (!reference_buffer) {
    printf(
  ...
  ioctl_result = bio_ioctl(device, BIO_IOCTL_GET_MEM_MAP, (void *)&devaddr);
  ...
  ioctl_result = bio_ioctl(device, BIO_IOCTL_PUT_MEM_MAP, nullptr);
  ...
  expected = test_buffer;
  actual = reference_buffer;
  for (uint i = 0; i < device->block_size; i++) {
    if (*actual != *expected) {
      printf(
          "Data mismatch at position %d. Expected %d got %d. "
