...
static struct bdevs {
  ...
  DECLARE_MUTEX(bdevs) lock;
} bdevs;

#define STACKBUF_DMA_ALIGN(var, size) \
  uint8_t var[ROUNDUP(size, MAX_CACHE_LINE)] __ALIGNED(MAX_CACHE_LINE);
...
static ssize_t bio_default_read(struct bdev *dev, void *_buf, off_t offset, size_t len) {
  ...
  block = static_cast<bnum_t>(offset / dev->block_size);
  ...
  if ((offset % dev->block_size) != 0) {
    ...
    /* copy what we need */
    block_offset = offset % dev->block_size;
    tocopy = ktl::min<size_t>(dev->block_size - block_offset, len);
  ...
  if (requires_alignment) {
    ...
    uint32_t num_blocks = divpow2(static_cast<uint>(len), static_cast<uint>(dev->block_shift));
...
static ssize_t bio_default_write(struct bdev *dev, const void *_buf, off_t offset, size_t len) {
  ...
  if ((offset % dev->block_size) != 0) {
    ...
    size_t tocopy = ktl::min<size_t>(dev->block_size - block_offset, len);
  ...
  if (requires_alignment) {
    ...
    uint32_t block_count = divpow2(static_cast<uint>(len), static_cast<uint>(dev->block_shift));
...
static ssize_t bio_default_erase(struct bdev *dev, off_t offset, size_t len) {
  ...
  off_t pos = offset;
  while (remaining > 0) {
    size_t towrite = ktl::min<size_t>(remaining, dev->block_size);
...
static void bdev_inc_ref(bdev_t *dev) {
  LTRACEF("Add ref \"%s\" %d -> %d\n", dev->name, dev->ref.load(), dev->ref.load() + 1);
  dev->ref.fetch_add(1);
}

static void bdev_dec_ref(bdev_t *dev) {
  int oldval = dev->ref.fetch_sub(1);

  LTRACEF("Dec ref \"%s\" %d -> %d\n", dev->name, oldval, dev->ref.load());
...
void bio_initialize_bdev(bdev_t *dev, const char *name, size_t block_size, bnum_t block_count,
  ...
  DEBUG_ASSERT(block_size && ispow2(static_cast<uint>(block_size)));

  list_clear_node(&dev->node);
  dev->name = strdup(name);
  dev->block_size = static_cast<uint>(block_size);
  dev->block_count = block_count;
  dev->block_shift = log2_floor(block_size);
