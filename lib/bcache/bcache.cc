...
bcache_t bcache_create(bdev_t *dev, size_t block_size, int block_count) {
  ...
  cache->blocks =
      static_cast<struct bcache_block *>(malloc(sizeof(struct bcache_block) * block_count));
  for (int i = 0; i < block_count; i++) {
    cache->blocks[i].ref_count = 0;
    cache->blocks[i].is_dirty = false;
    cache->blocks[i].ptr = new (&ac) uint8_t[block_size];
    ZX_ASSERT_MSG(ac.check(), "Failed to allocate memory for block %d", i);
...
static int flush_block(struct bcache *cache, struct bcache_block *block) {
  int rc = static_cast<int>(bio_write(
      cache->dev, block->ptr, (off_t)block->blocknum * cache->block_size, cache->block_size));
...
void bcache_destroy(bcache_t _cache) {
  struct bcache *cache = static_cast<struct bcache *>(_cache);
  int i;
  ...
  for (i = 0; i < cache->count; i++) {
    ...
    delete[] static_cast<uint8_t *>(cache->blocks[i].ptr);
  }
...
static struct bcache_block *find_or_fill_block(struct bcache *cache, uint blocknum) {
  ...
  if (block == nullptr) {
    ...
    err = static_cast<int>(
        bio_read(cache->dev, block->ptr, (off_t)blocknum * cache->block_size, cache->block_size));
    if (err < 0) {
...
int bcache_mark_block_dirty(bcache_t priv, uint blocknum) {
  int err;
  struct bcache *cache = static_cast<struct bcache *>(priv);
  struct bcache_block *block;
