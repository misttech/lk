...
static void set_cwd(const char *path) {
  ...
  char *new_cwd = static_cast<char *>(realloc(cwd, len));
  if (new_cwd) {
    cwd = new_cwd;
    memcpy(cwd, path, len);
  }
...
static int cmd_ls(int argc, const cmd_args *argv, uint32_t flags) {
  ...
  // construct the path
  char *path = static_cast<char *>(malloc(FS_MAX_PATH_LEN));
  ...
  pathlen = strlen(path);
  tmppath = static_cast<char *>(calloc(FS_MAX_PATH_LEN, 1));
...
static int cmd_cd(int argc, const cmd_args *argv, uint32_t flags) {
  if (argc < 2) {
    set_cwd(NULL);
  } else {
    char *path = static_cast<char *>(malloc(FS_MAX_PATH_LEN));
...
static int cmd_pwd(int argc, const cmd_args *argv, uint32_t flags) {
  puts(get_cwd());

  return 0;
}

static int cmd_mkdir(int argc, const cmd_args *argv, uint32_t flags) {
  if (argc < 2) {
...
static int cmd_mkfile(int argc, const cmd_args *argv, uint32_t flags) {
  if (argc < 2) {
    printf("not enough arguments\n");
...
static int cmd_rm(int argc, const cmd_args *argv, uint32_t flags) {
  if (argc < 2) {
    printf("not enough arguments\n");
    printf("usage: %s <path>\n", argv[0].str);
    return -1;
...
static int cmd_stat(int argc, const cmd_args *argv, uint32_t flags) {
  if (argc < 2) {
    printf("not enough arguments\n");
    printf("usage: %s <path>\n", argv[0].str);
    return -1;
...
static int cmd_cat(int argc, const cmd_args *argv, uint32_t flags) {
  status_t status = NO_ERROR;
  char buf[64];
  ssize_t read_len;
  off_t offset = 0;
...
static int cmd_df(int argc, const cmd_args *argv, uint32_t flags) {
  fs_dump_mounts();
  return NO_ERROR;
}
