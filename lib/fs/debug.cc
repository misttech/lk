...
static int cmd_fs_ioctl(int argc, const cmd_args *argv, uint32_t flags) {
  if (argc < 3) {
    printf("not enough arguments\n");
    return ERR_INVALID_ARGS;
  }

  int request = static_cast<int>(argv[2].u);
...
static int cmd_fs(int argc, const cmd_args *argv, uint32_t flags) {
  int rc = 0;
  ...
  if (!strcmp(argv[1].str, "mount")) {
    ...
    err = fs_format_device(argv[2].str, (argc >= 4) ? argv[3].str : nullptr, nullptr);
    ...
    return cmd_fs_ioctl(argc, argv, flags);
    ...
    if (argc < 5) {
      ...
      off = static_cast<off_t>(argv[4].u);
    }

    err = static_cast<int>(fs_write_file(handle, argv[3].str, off, strlen(argv[3].str)));
    if (err < 0) {
      printf("error %ld writing file\n", static_cast<long>(err));
      fs_close_file(handle);
      return err;
    }
