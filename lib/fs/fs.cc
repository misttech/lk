...
namespace {
DECLARE_SINGLETON_MUTEX(mount_lock);
...
extern const struct fs_impl __start_lk_fs[];
extern const struct fs_impl __stop_lk_fs[];

static const struct fs_impl *find_fs(const char *name) {
  for (const struct fs_impl *fs = __start_lk_fs; fs != __stop_lk_fs; fs++) {
...
void fs_dump_mounts(void) {
  printf("%-16s%s\n", "Filesystem", "Path");
  Guard<Mutex> guard(mount_lock::Get());
  struct fs_mount *mount;
...
status_t fs_open_file(const char *path, filehandle **handle) {
  ...
  fbl::AllocChecker ac;
  filehandle *f = new (&ac) filehandle;
  if (!ac.check()) {
    put_mount(mount);
    return ERR_NO_MEMORY;
  }
...
status_t fs_file_ioctl(filehandle *handle, int request, void *argp) {
  ...
  if (unlikely(!handle->mount || !handle->mount->api || !handle->mount->api->file_ioctl)) {
...
status_t fs_truncate_file(filehandle *handle, uint64_t len) {
  LTRACEF("filehandle %p, length %lu\n", handle, len);
...
ssize_t fs_load_file(const char *path, void *ptr, size_t maxlen) {
  ...
  ssize_t read_bytes = fs_read_file(handle, ptr, 0, ktl::min(maxlen, stat.size));
